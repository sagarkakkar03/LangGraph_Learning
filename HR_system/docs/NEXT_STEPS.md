...
**Expected:** You should see results with `country=Brazil` and doc codes starting with `BR-HR-*`. If you see `None` or `Argentina` results, the index is stale — delete `data/faiss_index/` and re-run `python scripts/embed_documents.py`.
...
- In the `run_worker.py` terminal, add this temporary debug line at the top of `process_email()`:
  ```python
  import os
...
Your personal email is not in the Postgres `employees` table. Update `data/employee_data.csv` with your sending email, then re-seed:
```bash
dropdb hr_system && createdb hr_system && python scripts/seed_db.py && python scripts/embed_documents.py
```
...
1. Check the `run_worker.py` logs — look for `department:` in the output.
...
1. Delete the old index: `rm -rf data/faiss_index/`
2. Rebuild: `python scripts/embed_documents.py`
...
- Run `python scripts/test_email_connection.py`.
...
To move this system from a local prototype to a robust, production-ready application, follow these steps:

### 1. Evaluate & Benchmark Performance (LangSmith)
Before deploying, you need to prove the system works reliably across edge cases.
- **Create a Dataset:** In the LangSmith UI, go to "Datasets & Testing" and create a new dataset named `HR_Email_Benchmark`.
- **Add Examples:** Add 20-50 diverse email examples.
  - *Inputs:* `query` (the email body), `sender_email`
  - *Outputs:* `expected_department`, `needs_escalation` (true/false), `reference_answer`
- **Write an Eval Script:** Create a script (e.g., `scripts/evaluate.py`) that uses LangSmith's `evaluate()` function to run your LangGraph workflow against this dataset.
- **Metrics:** Use LLM-as-a-judge evaluators to score the bot's answers for *Groundedness* (did it hallucinate?) and *Helpfulness*.

### 2. Implement Few-Shot Prompting
Zero-shot prompts (what we have now) are good, but few-shot prompts drastically reduce classification errors and improve tone.
- **Update `CLASSIFIER_PROMPT_TEMPLATE`:** Modify `app/core/config.py` to include 3-5 examples of tricky emails and how they should be routed.
  - *Example:* "Query: 'I need to add my newborn to my health plan' -> Output: Benefits"
- **Update RAG Generation:** Modify `generate_answer` in `rag_agent.py` to include an example of the desired tone, formatting, and how to properly cite document URLs.

### 3. Migrate Database to the Cloud
Local PostgreSQL is ephemeral. Move your data to a managed cloud provider.
- **Relational Data:** Provision a free-tier Postgres database on **Supabase** or **Neon**. Update your `.env` `DATABASE_URL` to point to the cloud instance. Run `python scripts/seed_db.py` to populate it.
- **Vector Store:** Local FAISS (`data/faiss_index/`) is not suitable for cloud deployments because containers are stateless. 
  - *Option A (Recommended):* Enable the `pgvector` extension in your new cloud Postgres database and migrate the FAISS logic to use `langchain_postgres`.
  - *Option B:* Use a managed vector database like **Pinecone** or **Qdrant**.

### 4. Containerization & Deployment
- **Dockerize:** Create a `Dockerfile` and `docker-compose.yml` that defines two services:
  1. The API Server (`uvicorn app.api.main:app`)
  2. The Background Worker (`python run_worker.py`)
- **Hosting:** Deploy the Docker containers to a platform like **Render**, **Railway**, or **AWS ECS**.
- **Secrets Management:** Move API keys and email credentials out of `.env` files and into the secure environment variable settings of your hosting provider.

### 5. Day-2 Operations (Pending Features)
- **Document Management API:** Currently, updating HR documents requires a developer to run `python scripts/embed_documents.py`. Add a `POST /documents/upload` endpoint in `main.py` that allows HR admins to upload new `.docx` files, automatically parse, chunk, embed, and update the database/vector store.
- **Conversation Memory:** Add a `conversations` table to Postgres to track email threads (using `Message-ID` and `In-Reply-To`), allowing the bot to remember context from previous exchanges.
- **Error Alerting:** Integrate Sentry or Datadog to alert you immediately if the IMAP connection drops or the OpenAI API rate limits are hit.
