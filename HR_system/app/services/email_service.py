...
def forward_to_department(
    ...
    emp_details = f"--- Employee Information ---\n{employee_info or 'Unknown / External'}\n\n"
    
    if escalation_reason:
        fwd_subject = f"[ESCALATION] {fwd_subject}"
        fwd_body = (
            f"Escalation Reason: {escalation_reason}\n\n"
            f"{emp_details}"
            f"--- Original query from {sender_email} ---\n\n{query}\n\n"
            f"--- Bot's Automated Response ---\n\n{response_body}"
        )
    else:
        fwd_body = f"{emp_details}Original query from {sender_email}:\n\n{query}"
